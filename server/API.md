...
1. **말로**: `listening` 상태의 **확정 문장(`final`)** 이 아래 조건에 맞으면 자동 전환된다 (공백은 무시하고 비교).

   ```
   외국인 · 외국어 · 통역  중 하나가 들어있거나
   "대화" + ("번역" 또는 "통역")  을 함께 말한 경우
   ```

   예: "통역 켜줘", "외국인이랑 대화 번역해줘", "대화 번역해줘" — dialog 진입.
   **"번역해줘"만으로는 진입하지 않는다** (이미지 번역과 섞이지 않게 좁혀둔 판정이다).
...
dialog 모드의 확정 문장이 아래 종료어 **하나와 정확히 같으면** 호출어 대기 상태로 돌아온다
(대소문자·앞뒤 공백·끝 구두점 `.!?,`는 무시. 문장 안에 섞여 있기만 하면 종료되지 않는다).

```
stop · exit · quit · finish · 종료 · 그만 · 끝
```
...
| "통역 켜줘", "외국인이랑 대화 번역" | `status: dialog` → 대화 번역 모드 | **사람**과의 대화 |

dialog 진입 판정이 먼저 돌고, 걸리지 않을 때만 기능 키워드를 본다. dialog 진입어가 좁게(외국인·외국어·
통역 / 대화+번역) 잡혀 있어서 "메뉴판 번역해줘"는 dialog로 새지 않고 이미지 번역으로 간다.

기능 키워드 매핑은 [`modules/stt/keyword_spotter.py`](modules/stt/keyword_spotter.py),
dialog 진입/종료 판정은 [`modules/stt/service.py`](modules/stt/service.py),
